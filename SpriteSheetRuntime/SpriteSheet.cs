...
namespace SpriteSheetRuntime
{
   ...
   /*     internal SpriteSheet(ContentReader input)
        ...
        {
            Texture = input.ReadObject<Texture2D>();
            spriteRectangles = input.ReadObject<List<Rectangle>>(); //input.ReadObject<Rectangle[]>();
            spriteNames = input.ReadObject<Dictionary<string, int>>();
        }
      ...
      /*  public Texture2D Texture
        ...
        public bool TryGetSourceRectangle(string spriteName, out Rectangle? spriteRect)
        {
            ...
            int spriteIndex;
            if (spriteNames.TryGetValue(spriteName, out spriteIndex))
            {
                if (spriteIndex >= 0)
                {
                    spriteRect = spriteRectangles[spriteIndex];
                    return true;
                }
                else
                {
        ...
        {
            if ((spriteIndex < 0) || (spriteIndex >= spriteRectangles.Count)) //Length))
