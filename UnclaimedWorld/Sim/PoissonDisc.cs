...
namespace UWGame.SimSide
{
    ...
    public static class UniformPoissonDiskSampler
    {
        ...
        static List<Vector2> Sample(Vector2 topLeft, Vector2 lowerRight, float? rejectionDistance, float minimumDistance, int pointsPerIteration)
        {
            ...
            {
                var listIndex = The.Sim.GameplayRandomGenerator.Next(state.ActivePoints.Count,"PoissonDisc");
        ...
        static void AddFirstPoint(ref Settings settings, ref State state)
        {
            ...
            while (!added)
            {
                var d = The.Sim.GameplayRandomGenerator.NextDouble("PoissionDisc");
                var xr = settings.TopLeft.X + settings.Dimensions.X * d;

                d = The.Sim.GameplayRandomGenerator.NextDouble("PoissionDisc");
